...
/*		if ( $this->listing_id ) {
	...
	public function process() {
		...
		if(isset( $_POST[ 'listeo_core_form' ] )) {
			listeo_write_log($this->step);
			listeo_write_log($this->steps);
	...
	public function init_fields() {
		...
		} elseif(isset($_POST["_listing_type"])){
		...
		$this->fields = apply_filters('submit_listing_form_fields', array(
			'basic_info' => array(
				...
				'fields' 	=> array(
						...
						'listing_category' => array(
							...
							'type'        => 'term-select',
							'placeholder' => '',
							'name'        => $listing_type=="event"?"event_category":'listing_category',
							'taxonomy'	  => $listing_type=="event"?"event_category":'listing_category',
			...
			'partners' => array(
				'title' 	=> __('Nos partenaires','listeo_core'),
				'class' 	=> 'margin-top-40',
				'icon' 		=> 'sl sl-icon-doc',
				'fields' 	=> array(
					'producteurs' => array(
						'label'       => __( 'Producteurs', 'listeo_core' ),
						'type'        => 'text',
						'name'       => 'producteurs',
						'tooltip'	  => __( 'Quels sont vos producteurs ?', 'listeo_core' ),
						// 'required'    => false,
						// 'placeholder' => '',
						'class'		  => 'tags',
						'priority'    => 1,
					...
					'transformateurs' => array(
						'label'       => __( 'Transformateurs', 'listeo_core' ),
						'type'        => 'text',
						'name'       => 'transformateurs',
						'tooltip'	  => __( 'Quels sont vos transformateurs ?', 'listeo_core' ),
						'required'    => false,
						// 'placeholder' => '',
						'class'		  => 'tags',
						'priority'    => 1,
				...
				foreach ( $this->fields as $group_key => $group_fields ) {
					foreach ( $group_fields['fields'] as $key => $field ) {
						...
						switch ( $key ) {
							...
							default:
							// if($key=="producteurs"){
							// 	var_dump($this->fields[ $group_key ]['fields'][ $key ]['value'])
							// var_dump(get_post_meta( $listing->ID, $key, true ));exit;}
...
// 		var_dump($this->fields);
		...
		// $category = get_term_by('slug', $item, 'listing_category');
...
$js = "var options = ";
$items = $this->fields["partners"]["fields"]["producteurs"]["value"];
$item_datas = array();
if(!empty($items)) {$items = explode(',',$items);
	foreach($items as $item){
		if(is_numeric($item)){
			$post = $this->get_post_by_id($item);
			if($post){
				$item_datas[] = array("id"=>$post->ID, "title"=>$post->post_title);
			}
		} else $item_datas[] = array("id"=>$item, "title"=>$item);
	}
}
$js.=json_encode($item_datas);
$js.=";";
$js.="var items = options.map(x => x.id);
jQuery('#producteurs').selectize({
  options: options,
  items: items,
  valueField: 'id',
  labelField: 'title',
  ...
  create: true,
    ...
    load: function(query, callback) {
        ...
        jQuery.ajax({
            url: '".home_url( $wp->request )."?jsonify=1&listings=1',
            type: 'GET',
            data: {
                q: query,
                page_limit: 10,
...
$js = "var options = ";
$items = $this->fields["partners"]["fields"]["transformateurs"]["value"];
$item_datas = array();
if(!empty($items)) {$items = explode(',',$items);
	foreach($items as $item){
		if(is_numeric($item)){
...
$js.=";";
$js.="var items = options.map(x => x.id);
jQuery('#transformateurs').selectize({
  options: options,
  items: items,
  valueField: 'id',
  labelField: 'title',
...
//         if (!query.length) return callback();
//         jQuery.ajax({
//             url: 'http://localhost/malt/andre/mf/add-listing/?jsonify=1&listings=1',
//             // url: 'http://localhost/malt/andre/mf/add-listing/?jsonify=1&parent=64&feature=1',
//             type: 'GET',
//             data: {
//                 q: query,
//                 page_limit: 10,
