...
class Listeo_Core_Listing {
	...
	public function add_query_vars($vars) {
        ...
        array_push($new_vars, 'date_range', 'keyword_search','location_search','listeo_core_order','search_radius','radius_type','location_var','departement');
	...
	public static function get_real_listings($args ) {
		...
		if ( isset($args['location']) && !empty($args['location']) ) {
			...
			listeo_write_log($radius_type);
			listeo_write_log($radius_api_key);
			if(!empty($args['location']) && !empty($radius) && !empty($radius_api_key)) {
				//search by google
				listeo_write_log('werehere');
		...
		if(isset($args['date_start']) && !empty($args['date_start'])) {
			...
			if(!empty($args['_listing_type'])&&$args['_listing_type']=="event"){

		$date_start = esc_sql ( date( "Y-m-d H:i:s", strtotime( $args['date_start'] ) ) );
	    $date_end = esc_sql ( date( "Y-m-d H:i:s", strtotime( $args['date_end'] ) ) );
        ...
        foreach ($taxonomy_objects as $tax) {
			...
			$get_tax = false;
			if(!empty(get_query_var( 'tax-'.$tax->name ))){
...
// var_dump($get_tax);
        	if(is_array($get_tax)){
        		...
        		$query_args['tax_query'][$tax->name] = array('relation'=> 'AND');
		...
		foreach ($available_query_vars as $key => $meta_key) {
			...
			if($meta_key =="_listing_type"){
				...
				if(!empty($meta))
				$query_args['meta_query']['_listing_type'] = array(
		            'key'     => '_listing_type',
					'value'   => $meta,
					'compare' => '='
		        );	
...
/*						$query_args['meta_query']['featured'] = array(
   ...
   //  	if( !empty($args['departement'])){
			// array_push($query_args['tax_query'], array(
	  //          'taxonomy' =>  "region",
	  //          'field'    =>  'slug',
	  //          'terms'    =>  $args['departement'],
	  //          'operator' =>  'IN'
	  //       ));	
   //  	}
