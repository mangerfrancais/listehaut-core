		...
		ob_start(); ?>
			...
			<ul id="listeo-activities-list">
				...
				if($items) :
					...
					$end = '<span class="activity-time">Il y a '.human_time_diff( $item->log_time, current_time('timestamp') ) . '</span>';
					...
					switch ($item->action) {
							...
							case 'unbookmarked':
								echo $start.'
									<i class="list-box-icon sl sl-icon-star"></i> '.esc_html__('Someone unbookmarked your','listeo_core').' <strong><a href="'.esc_url($post_url).'">'.$post_title.'</a></strong> listing!
								'.$end;
								break;
