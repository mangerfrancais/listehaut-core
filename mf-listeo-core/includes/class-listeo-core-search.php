...
//var_dump($this->mf_select);exit;
		...
		// }else{
			add_action( 'wp_ajax_nopriv_listeo_get_features_from_category', array( $this, 'ajax_get_features_from_category2' ) );
			add_action( 'wp_ajax_listeo_get_features_from_category', array( $this, 'ajax_get_features_from_category2' ) );
		// }
...
global $wpdb;
	...
	if(!empty($date_range)) : 
	    ...
	    $where .= $GLOBALS['wpdb']->prepare( ' AND wp_posts.ID '.
	        'NOT IN ( '.
	            'SELECT listing_id '.
	        ...
	        ' ) ', $date_start, $date_start, $date_end,  $date_end, $date_start, $date_end );
	...
	public function pre_get_posts_listings( $query ) {
		...
		if ( is_tax('listing_category') || is_tax('listing_feature')  || is_tax('listing_region') ) {
			$per_page = get_option('listeo_listings_per_page',10);
		    $query->set( 'posts_per_page', $per_page );	
		}
		...
		if ( is_post_type_archive( 'listing' ) || is_author() || is_tax('listing_category') || is_tax('listing_feature') || is_tax('event_category') || is_tax('service_category') || is_tax('rental_category') || is_tax('region')) {
			...
			if( $location ) {
				...
				if(!empty($location) && !empty($radius) && !empty($radius_api_key)) {
					...
					//search by google
					listeo_write_log('tu jestem');
					$latlng = listeo_core_geocode($location);

					listeo_write_log($latlng);
...
public function ajax_get_listings() {
		...
		$departement  	= (isset($_REQUEST['departement'])) ? sanitize_text_field( stripslashes( $_REQUEST['departement'] ) ) : '';
		...
		$query_args['listeo_orderby'] = sanitize_text_field( $_REQUEST['listeo_core_order'] );
						...
						foreach ($groups as $group) { ?>
							...
							<?php foreach ($group as $feature) { 
								...
								<div class="panel-checkbox-wrap">
									<input id="<?php echo esc_html($feature) ?>" value="<?php echo esc_html($feature) ?>" type="checkbox" name="tax-listing_feature[<?php echo esc_html($feature); ?>]">
	...
	public function ajax_get_features_from_category2(){
		...
		if($categories){
			...
			if($features){
				if($panel != 'false'){ ?>
					<div class="mf-ajax panel-checkboxes-container">
					<?php
						$groups = array_chunk($features, 4, true);	
						foreach ($groups as $group) { ?>
							<?php foreach ($group as $feature) { 
								...
								?>
								<div class="panel-checkbox-wrap">
									<input class="mf-tax-listing_feature" id="<?php echo esc_html($feature) ?>" value="<?php echo esc_html($feature) ?>" type="checkbox" name="tax-listing_feature[<?php echo esc_html($feature); ?>]">
									<label for="<?php echo esc_html($feature) ?>"><?php echo $feature_obj->name; ?></label>	
								</div>
							<?php } ?>
						<?php } ?>
				...
				<?php } else {
					...
					foreach ($features as $feature) { 
						...
						$children = get_term_children($feature_obj->term_id,'listing_feature');
						if(empty($children)){
							$ii++;?>
							<div class="checkboxes mf-ajax mf-ajax<?php echo $ii?>">
								<input class="mf-tax-listing_feature" id="<?php echo esc_html($feature) ?>" value="<?php echo esc_html($feature) ?>" type="checkbox" name="tax-listing_feature[<?php echo esc_html($feature); ?>]">
								<label class="" for="<?php echo esc_html($feature) ?>"><?php echo $feature_obj->name; ?></label>
							</div>
						<?php } elseif(count($children)>0){
							if(count($children)<2){
								$ii++;								
							foreach ($children as $child) { 
								$child_obj = get_term_by('id', $child, 'listing_feature');
								if( !$child_obj ){
									continue;
								}
								?>
							<div class="checkboxes mf-ajax mf-ajax<?php echo $ii?>">
								<input class="mf-tax-listing_feature" value="<?php echo esc_html($child_obj->name) ?>" type="checkbox" id="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" name="tax-listing_feature[<?php echo esc_html($child_obj->name); ?>]" >
								<label class="" id="label-in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" for="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>"><?php echo $child_obj->name; ?></label>
							</div>
								<?php 
							} }
							else { 
								...
								foreach ($children as $child) { 
									...
									$datas[esc_html($child_obj->term_id)] = esc_html($child_obj->name);?>
									<input style="display: none" value="<?php echo esc_html($child_obj->name) ?>" type="checkbox" id="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" name="tax-listing_feature[<?php echo esc_html($child_obj->name); ?>]" class="mf-tax-listing_feature in-listing_feature-<?php echo $feature_obj->term_id?>">
									<?php
								} 
								if(empty($datas)) continue; ?>
								<div class="mf-ajax row with-forms mf__category">
								   <div class="col-md-12">
								      <a href="#" class="more-search-options-trigger" data-open-title="<?php echo esc_html($feature_obj->name) ?>" data-close-title="<?php echo esc_html($feature_obj->name) ?>"></a>
								   </div>
								   <div class="col-md-12 more-search-options" style="display:none">
								      <div class="more-search-options-container row">
								         <div class="col-md-12 ">
								            <select <?php /*name="listing_feature[<?php echo esc_html($feature_obj->name) ?>]" id="a-in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>"*/?>
								               data-placeholder=" " class="chosen-select mf-chosen-select" data-parent="in-listing_feature-<?php echo $feature_obj->term_id?>">
								               <option value="">---</option>
				...
				<?php } ?>
				<script type="text/javascript">
					jQuery('.mf-chosen-select').off('change');
					jQuery('.mf-chosen-select').on('change', function(e){
				    	e.preventDefault();
				        jQuery(".mf-tax-listing_feature").prop("checked",false);
				        jQuery('.mf-chosen-select').each(function( index ) {
				        	jQuery('.'+jQuery(this).data("parent")).prop("checked",false);
				        	jQuery('#in-listing_feature-'+jQuery(this).val()).prop('checked',true);
				        	console.log(jQuery(this).val());
						});
				    });
					jQuery('.chosen-select').chosen({disable_search_threshold: 10, width:"100%"});
				    jQuery('.more-search-options-trigger').off('click');
				    jQuery('.more-search-options-trigger').on('click', function(e){
				    	e.preventDefault();
						jQuery(this).toggleClass('active');
						jQuery(this).parent().parent().find('.more-search-options').animate({height: 'toggle', opacity: 'toggle'}, 300);
					});
				</script>
				<?php
			} else { 
			...
			$success = false; ?>
			<?php /*<div class="notification warning"><p><?php esc_html_e('Please choose category to display filters','listeo_core') ?></p> </div>*/?>
		<?php }
		...
		if($categories){
			...
			if($features){
				...
				<?php } else {
					...
					foreach ($features as $feature) { 
						// var_dump(in_array(  $feature_obj->term_id, $selected ));exit;
						$feature_obj = get_term_by('slug', $feature, 'listing_feature');
						if( !$feature_obj ){
							continue;
						}
						...
						<?php } elseif(count($children)>0){?>
							<?php /*<input <?php checked( in_array(  $feature_obj->term_id, $selected ) ); ?>value="<?php echo esc_html($feature_obj->term_id) ?>" type="checkbox" id="in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>" name="tax_input[listing_feature][]" >

							<label id="label-in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>" for="in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>"> <?php echo $feature_obj->name; ?></label><br>*/?>
							<p class="parent-part" id="label-in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>" for="in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>"> <?php echo $feature_obj->name; ?></p>
							<?php 
								if(count($children)<2)
								foreach ($children as $child) { 
									$child_obj = get_term_by('id', $child, 'listing_feature');
									...
									<input <?php checked( in_array(  $child_obj->term_id, $selected ) ); ?> value="<?php echo esc_html($child_obj->term_id) ?>" type="checkbox" id="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" name="tax_input[listing_feature][]" >
									<label id="label-in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" for="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>"><?php echo $child_obj->name; ?></label>					
									<?php 
								} else {?>
									<input type="text" id="in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>">
								<?php 
								foreach ($children as $child) { 
									$child_obj = get_term_by('id', $child, 'listing_feature');
									if( !$child_obj ){
									...
									<input <?php checked( in_array(  $child_obj->term_id, $selected ) ); ?> value="<?php echo esc_html($child_obj->term_id) ?>"  class="to-feature" data-class="ajax-feature" type="checkbox" style="display:none" id="in-listing_feature-<?php echo esc_html($child_obj->term_id) ?>" name="tax_input[listing_feature][]" >
								<?php } ?>
							<script type="text/javascript">
							jQuery(function($) { 
							var in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>_options = <?php 
							$item_datas = array();
							foreach ($children as $child) { 
								$child_obj = get_term_by('id', $child, 'listing_feature');
								if( !$child_obj ){
								...
								if(in_array(  $child_obj->term_id, $selected )){
									$item_datas[] = array("id"=>esc_html($child_obj->term_id), "title"=>esc_html($child_obj->name));
								}
							}

							$js =json_encode($item_datas);
							$js.=";";
							echo $js;
							?>
							var in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>_items = in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>_options.map(x => x.id);
							if(typeof in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?> !== "undefined") in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>[0].selectize.remove();
							var in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?> = 
							jQuery('#in-listing_feature-<?php echo esc_html($feature_obj->term_id) ?>').selectize({
							  options: in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>_options,
							  items: in_listing_feature_<?php echo esc_html($feature_obj->term_id) ?>_items,
							  valueField: 'id',
							  labelField: 'title',
							  searchField: ['title'],
							  persist: false,
							  ...
							  create: false,
							   onItemAdd: function (value) {
							        jQuery("#in-listing_feature-"+value).prop("checked",true);
							    },							  
							   onItemRemove: function (value) {
							        jQuery("#in-listing_feature-"+value).prop("checked",false);
							    },							  
							    render: {
							    ...
							    load: function(query, callback) {
							        ...
							        jQuery.ajax({
							            url: '<?php echo home_url( $wp->request )."?jsonify=1&feature=1&parent="?><?php echo esc_html($feature_obj->term_id) ?>',
							            type: 'GET',
							            data: {
							                q: query,
							                page_limit: 10,
						...
						<?php }?>
							...
							jQuery(function($) {
								// jQuery(".ajax-feature").remove();
								// jQuery(".to-feature").addClass("ajax-feature");
							});
							</script>*/?>
					<?php }
				}
  ...
  // 2 => string 'radius'
	...
	public function output_search_form( $atts = array() ){
		if(!empty($atts["mf_select"]))
			$this->mf_select = (bool)$atts["mf_select"];
		// var_dump($this->mf_select);exit;
...
//var_dump(get_query_var( '_listing_type' ));exit;
		// var_dump($search_fields);exit;
		switch (get_query_var( '_listing_type' )) {

			case 'event':
			if(!empty($search_fields["keyword_search"]))
				unset($search_fields["keyword_search"]);
			if(!empty($search_fields["tax-listing_category"]))
				unset($search_fields["tax-listing_category"]);
			if(!empty($search_fields["tax-listing_feature"]))
				unset($search_fields["tax-listing_feature"]);
			
			$search_fields["tax-event_category"]["css_class"]="col-md-12";
			break;
			...
			default:
				if(!empty($search_fields["date_range"]))
					unset($search_fields["date_range"]);
				if(!empty($search_fields["tax-event_category"]))
					unset($search_fields["tax-event_category"]);
				if(!(empty(get_query_var( "tax-listing_category" ))))
					unset($search_fields["tax-listing_feature"]);
				break;

		}		

		// unset($search_fields["location_search"]);
		// var_dump($search_fields);exit;
		...
		<?php }
			...
			foreach ($search_fields as $key => $value) {
				...
				if(isset($value['place']) && $value['place'] == 'main') {
					...
					if($source == 'half') {
						...
						if($temp_count==0 && $value["name"]!== "tax-listing_feature"){
							echo '<div class="row with-forms mf_'.($value['name']).'">';
						}
						if($this->mf_select && $value['name'] == 'tax-listing_category'){?>
							<div class="col-md-12">
							<a href="#" class="more-search-options-trigger" data-open-title="Type de produit" data-close-title="Type de produit"></a></div>
							<div class="col-md-12 more-search-options " style="display:none">
								<div class="more-search-options-container row">
						<?php }						
						...
						<?php }					
						if($temp_count==$count && $value["name"]!== "tax-listing_feature"){
							echo '</div>';	
						}
					} else {

						if($source == 'sidebar' && $value["name"]!== "tax-listing_feature") { 
							if($value["name"]=="location_search")
							echo '<div class="row with-forms"><div class="col-md-7">';
							echo '<div class="row with-forms mf_'.($value['name']).'">'; }
						if($this->mf_select && $value['name'] == 'tax-listing_category'){?>
							<div class="col-md-12">
							<a href="#" class="more-search-options-trigger" data-open-title="Type de produit" data-close-title="Type de produit"></a></div>
						...
						<?php }	
							...
							$template_loader->set_template_data( $value )->get_template_part( 'search-form/'.$value['type']);
						if($source == 'sidebar' && $value["name"]!== "tax-listing_feature") { echo '</div>'; }
						if($this->mf_select && $value['name'] == 'tax-listing_category'){?>
								</div>
							</div>
						<?php }	
						if($value["name"]=="location_search")
						echo '</div><div class="col-md-1"><p class="or-dada">ou</p></div><div class="col-md-4">';
						if($value["name"]=="tax-region")
						echo '</div></div>';
					...
					if($value['type'] == 'radius') { ?>
						<div class="row with-forms <?php echo ' mf_'.$value['key'].'' ?>">
							<div class="col-md-12">
								<span class="panel-disable" data-disable="<?php echo esc_attr_e( 'Disable Radius', 'listeo_core' ); ?>" data-enable="<?php echo esc_attr_e( 'Enable Radius', 'listeo_core' ); ?>"><?php esc_html_e('Disable Radius'); ?></span>
							</div>
						</div>
			...
			<?php if($more_trigger == 'yes') : ?>				
				<!-- More Search Options -->
				<?php /*<a href="#" class="more-search-options-trigger <?php echo esc_attr($more_custom_class) ?>" data-open-title="<?php echo esc_attr($more_text_open) ?>" data-close-title="<?php echo esc_attr($more_text_close) ?>"></a>*/?>
