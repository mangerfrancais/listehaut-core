...
if($("#avatar-uploader").length>0) {
  ...
  $('.dynamic #tax-listing_category,.dynamic #tax-listing_category-panel input').on('change',function(e) {
      ...
      if(!$('#listeo_core-search-form').hasClass('main-search-form'))
      $.ajax({
          type: 'POST', 
          dataType: 'json',
          url: listeo.ajaxurl,
...
//            $('#tax-listing_feature-panel .checkboxes .row').html(data['output']).removeClass('loading');
            ...
            $('#listeo_core-search-form .mf-ajax').remove();
            $('#listeo_core-search-form').append(data['output']);

            if($('#listeo_core-search-form').hasClass('has_category')){
              $('.row.with-forms.mf_tax-listing_category .more-search-options-trigger').addClass("active");
              $('.row.with-forms.mf_tax-listing_category .more-search-options').show();
            }

            // $('.row.with-forms.mf_tax-listing_category .more-search-options-trigger').trigger("click");
...
//            }
  ...
  if($('select#tax-listing_category').length>0)
  $('select#tax-listing_category').trigger('change');

  if($('#tax-listing_category-panel input:checked').length>0)
  $('#tax-listing_category-panel input:checked').trigger('change');
  ...
  //YYYY-MM-DD HH:MM:00
    ...
    $('.date_range').on('apply.daterangepicker', function(ev, picker) {
        ...
        $(this).val( picker.startDate.format("YYYY-MM-DD 00:00:00") + ' - ' + picker.endDate.format("YYYY-MM-DD 00:00:00")).trigger("change");
    });
